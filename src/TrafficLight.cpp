...
T MessageQueue<T>::receive()
{
    ...
    _cv.wait(u_lck, [this]{ return !_queue.empty(); });
...
void MessageQueue<T>::send(T &&msg)
{
    ...
    std::lock_guard<std::mutex> u_lock(_mtx);
...
void TrafficLight::waitForGreen()
{
    ...
    while(1)
    {
        auto _currentPhase = msg_queue_->receive();
...
void TrafficLight::cycleThroughPhases()
{
  ...
  while (true)
    {
        // Generate a random duration for this cycle (in seconds)
        int cycleDuration = distr(gen);

        // Measure the start time of the cycle
        ...
        std::chrono::duration<double> elapsed_time = now-lastUpdate;
    }
